...
startCookingBtn.addEventListener('click', () => {
  ...
  // Highlight steps + update progress bar
...
function submitQuery() {
  const message = document.querySelector('.query-box textarea').value;
  if (message.trim() === '') {
    alert('Please type something before submitting.');
  } else {
    alert('Thanks! We received your note: ' + message);
    document.querySelector('.query-box textarea').value = ''; // clear input
  }
}
